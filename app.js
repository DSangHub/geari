const LANGS = [
    ...
    const savedLang = localStorage.getItem("geari.lang");
    if (savedLang && LANGS.some(([c]) => c === savedLang)) sel.value = savedLang;
    applyLang(sel.value);
    sel.addEventListener("change", e => {
      localStorage.setItem("geari.lang", e.target.value);
    ...
    function tick() {
      ...
      log.innerHTML = events.map(e => row(e.from, e.to, e.pts, e.grade)).join("") ||
